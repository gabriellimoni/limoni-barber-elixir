defmodule LimoniBarberWeb.PatientLive.FormComponent do
  ...
  def update(%{patient: patient} = assigns, socket) do
    changeset = Attendances.change_patient(patient)
  ...
  def handle_event("validate", %{"patient" => patient_params}, socket) do
    changeset =
      ...
      |> Attendances.change_patient(patient_params)
  ...
  defp save_patient(socket, :edit, patient_params) do
    case Attendances.update_patient(socket.assigns.patient, patient_params) do
      {:ok, _patient} ->
        {:noreply,
         socket
  ...
  defp save_patient(socket, :new, patient_params) do
    case Attendances.create_patient(patient_params) do
      {:ok, _patient} ->
        {:noreply,
         socket
